...
import numpy as np
from batchgenerators.utilities.file_and_folder_operations import subfiles, isfile, load_pickle, join
from c_unet.preprocessing.experiment_planning.common_utils import get_pool_and_conv_props_poolLateV2
from c_unet.preprocessing.paths import default_data_identifier
from c_unet.preprocessing.crop import get_case_identifier_from_npz
from c_unet.preprocessing.plan import recursive_find_python_class
from c_unet.training.Generic_UNet import Generic_UNet


# MS Dropped(not even by me): determine_postprocessing "Spoiler: This is unused, postprocessing was removed. Ignore it."
...
class ExperimentPlanner(object):
    ...
    def determine_whether_to_use_mask_for_norm(self):
        ...
        for i in range(num_modalities):
            if "CT" in modalities[i]:
                ...
                if np.median(all_size_reductions) < 3 / 4.:  #MS no reduction = 1.0, so < 3/4 means a reduction of more than 1/4
    ...
    def run_preprocessing(self, num_threads, default_num_threads):
        ...
        # MS: finds class from preprocessing.py (e.g. PreprocessorFor2D for 2D).
        # print("Hi",self.preprocessor_name)
        preprocessor_class = recursive_find_python_class(
            [join('c_unet', "preprocessing")],self.preprocessor_name, current_module="c_unet.preprocessing"
        )
