...
from c_unet.training.Bayesian_UNet import *
...
class Predictor(object):
    def __init__(self, plans_file, encoderType, architecture_name,stage=None, deterministic=False, fp16=True):
    ...
    def initialize_network(self):
        print('DIMENSIONALITY',self.threeD)
        if self.threeD:
            conv_op = nn.Conv3d
        ...
        m = importlib.import_module(f"training.{self.architecture_name}")
        architecture_class = getattr(m, self.architecture_name)
        print(architecture_class)

        if self.architecture_name == "Bayesian_UNet":
            self.network = architecture_class(
                self.num_input_channels, self.base_num_features, self.num_classes,len(self.net_num_pool_op_kernel_sizes),
                self.conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,dropout_op_kwargs,net_nonlin, net_nonlin_kwargs, 
                True, False, lambda x: x, InitWeights_He(1e-2),self.net_num_pool_op_kernel_sizes, self.net_conv_kernel_sizes, 
                False, True, True
            )
            # network = Bayesian_UNet(
                # num_input_channels, base_num_features, num_classes,len(net_num_pool_op_kernel_sizes),
                # conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,dropout_op_kwargs,
                # net_nonlin, net_nonlin_kwargs, deep_supervision, False, lambda x: x, InitWeights_He(1e-2),
                # net_num_pool_op_kernel_sizes, net_conv_kernel_sizes, False, True, True
            # )
        elif self.architecture_name == "Generic_UNet": 
        ...
        elif self.architecture_name == "SMP_UNet":
            depth = len(self.net_num_pool_op_kernel_sizes)
            if depth>5:
                depth=5
            elif depth<3:
                depth = 3
            self.network = architecture_class(self.encoderType,self.num_input_channels, self.num_classes, depth,base_num_features = self.base_num_features)
        elif self.architecture_name == "MONAI_UNet":
            spatial_dims = 3 if self.threeD else 2
            self.network = architecture_class(self.encoderType,self.num_input_channels, self.num_classes,self.patch_size,spatial_dims)
        else:
            print("undefined notwork type")
            exit()

        print('PREDICTOR.PY CONV_OP',self.network.conv_op)
        if torch.cuda.is_available():
            self.network.cuda()
    ...
    def preprocess_patient(self, input_files):
        ...
        d, s, properties = preprocessor.preprocess_test_case(input_files,self.plans['plans_per_stage'][self.stage]['current_spacing'])
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        ret = self.network.predict_3D(
            data, do_mirroring, mirror_axes, use_sliding_window, step_size, self.patch_size,self.regions_class_order, 
            use_gaussian, pad_border_mode, pad_kwargs,all_in_gpu, verbose, mixed_precision=mixed_precision
        )
