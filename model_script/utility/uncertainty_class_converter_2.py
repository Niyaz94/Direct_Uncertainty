...
# datasets = ['KIDNEY','BRAIN_TUMOR','KNEE','PANCREATIC_LESION','PROSTATE','BRAIN_GROWTH','PANCREAS','SIJ','LUNG']
datasets = ['BRAIN_TUMOR_TASK1']

ROOT_PATH ='/U-Net/data'

TH = 0.01

for dataset in datasets:
    
    fname=f"{ROOT_PATH}/{dataset}"
    ...
    for mode in modes:
        ...
        for case in cases:
            print(case)
            cname_pattern = fname + '/MULTIPLE/images' + mode + '/' + case + '_0_000*.nii.gz'
            dname_pattern = fname + '/UNCERTAINTY_CLASS/images' + mode + '/'

            # Ensure destination exists
            os.makedirs(dname_pattern, exist_ok=True)

            # Loop through all matching files and copy them
            for file_path in glob.glob(cname_pattern):
                shutil.copy(file_path, dname_pattern)
