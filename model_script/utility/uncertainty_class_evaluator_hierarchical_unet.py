...
datasets = ['SIJ','LUNG','HEART','PANCREATIC_LESION']
...
for dataset in datasets:
    ...
    for ncase, case in enumerate(cases):
        ...
        ev_ims = [nib.load(pred_dir + dataset + model + 'hier_probabilistic/fold' + str(i) + '/' + case).get_fdata() for i in range(5)]
        #majority voting
        one_hot_ev_ims = np.asarray([[im==label for label in labels] for im in ev_ims],dtype=np.uint8)
        one_hot_ev_ims = np.sum(one_hot_ev_ims,axis = 0)
        one_hot_ev_ims = np.argmax(one_hot_ev_ims,axis=0)
        ev_im = np.copy(one_hot_ev_ims)

        single_ims = [nib.load(pred_dir + dataset + '/SINGLE/hier_probabilistic/fold' + str(i) + '/' + case).get_fdata() for i in range(5)]
        single_ims = np.mean(np.asarray(single_ims,dtype=np.float32),axis = 0)
        mean_labels = np.unique(single_ims)
        new_labels = [i for i in mean_labels if np.min([abs(i-l) for l in labels]) > TH]
        for i in new_labels:
            single_ims[single_ims==i] = np.max(labels)


        multiple_ims = [nib.load(pred_dir + dataset + '/MULTIPLE/hier_probabilistic/fold' + str(i) + '/' + case).get_fdata() for i in range(5)]
    ...
    fname = save_dir + dataset + '_direct_hier_probabilistic.txt'
    f = open(fname,'w')
    ...
    fname = save_dir + dataset + '_single_hier_probabilistic.txt'
    f = open(fname,'w')
    for i in range(len(cases)):
        for j in range(len(labels)-1):
    ...
    fname = save_dir + dataset + '_multiple_hier_probabilistic.txt'
    f = open(fname,'w')
    for i in range(len(cases)):
        for j in range(len(labels)-1):
    ...
    fname = save_dir + 'medians_hier_probabilistic.txt'
    f = open(fname,'a')
    print('##############################',file=f)
    ...
    fname = save_dir + 'stat_test_hier_probabilistic.txt'
    f = open(fname,'a')
    print('########################',file=f)
